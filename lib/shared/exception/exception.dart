...
class ExceptionView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Center(
      ...
      DataErrorStateType.somethingWentWrong ||
      DataErrorStateType.none ||
      DataErrorStateType.fetchData =>
        _buildUnknownException(),
  ...
  Widget _buildTitleWithImage({required String title, required String image}) {
    return LayoutBuilder(builder: (context, BoxConstraints constraints) {
      double? size = DeviceConfiguration.isMobileResolution
          ? null
