...
class SqfLiteDbHandler {
  ...
  static Future<SqfLiteDbHandler> initializeDb(DbInfo dbInfo) async {
    ...
    await component._initializeDb(
        dbInfo.dbName, dbInfo.queryFileName, dbInfo.dbVersion);
    return component;
  }

  Future<void> _initializeDb(
      String dbName, String queryFileName, int dbVersion) async {
    try {
      ...
      _db = await openDatabase(dbPath,
          ...
          onCreate: (Database db, int version) =>
              _onCreate(db, version, queryFileName),
  ...
  Future<dynamic> query(String tableName,
      {String? columnName, List<String>? ids}) async {
    return await db.query(tableName,
        where: columnName != null ? '$columnName = ?' : null, whereArgs: ids);
  }

  Future<dynamic> rawQueryWithParams(String query,
  ...
  Future<List<Object?>> insertBulkData(
      String tableName, List<Map<String, dynamic>> bulkData) async {
    Batch batch = db.batch();
  ...
  Future<bool> deleteTableRowsBasedOnTheDate(int millisecondsSinceEpoch,
