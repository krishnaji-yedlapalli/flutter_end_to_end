...
abstract class DailyTrackerRepo {
  ...
  Future<DailyTrackerEventModel> createOrEditEvent(
      DailyTrackerEventModel event);
...
class DailyTrackerRepository extends DailyTrackerRepo {
  ...
  DailyTrackerRepository({BaseService? baseService})
  ...
  Future<(bool, List<DailyTrackerEventModel>)> isCheckedIn(String date) async {
    ...
    var response =
        await baseService.makeRequest(url: '${Urls.dailyCheckIns}/$date.json');
    if (response != null && response['events'] != null) {
      events = response['events']
          .map<DailyTrackerEventModel>(
              (json) => DailyTrackerEventModel.fromJson(json))
          .toList();
    }
  ...
  @override
  Future<DailyTrackerEventModel> createOrEditEvent(
      DailyTrackerEventModel event) async {
    var body = {event.id: event.toJson()};

    var response = await baseService.makeRequest(
        url: '${Urls.events}.json', body: body, method: RequestType.patch);
  ...
  Future<List<DailyTrackerEventModel>> fetchEventList() async {
    ...
    if (response is Map) {
      events = response.entries
          .map<DailyTrackerEventModel>(
              (json) => DailyTrackerEventModel.fromJson(json.value))
          .toList();
  ...
  Future<bool> deleteEvent(String eventId) async {
    var status = await baseService.makeRequest(
        url: '${Urls.events}/$eventId.json', method: RequestType.delete);
    return true;
  }
}
