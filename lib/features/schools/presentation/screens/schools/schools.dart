  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(
        ...
        actions: [
          featureDiscovery(() =>
              SchoolScreenFeatureDiscovery().startFeatureDiscovery(
                  context, forceTour: true))
        ],
        appBar: AppBar(),
      ),
      floatingActionButton: SchoolScreenFeatureDiscovery().aboutSchoolDiscovery(
          type: SchoolDiscoverFeatureType.create,
          child: FloatingActionButton.extended(onPressed: onTapOfCreateSchool,
              label: const Text('Create School'),
              icon: const Icon(Icons.add))),
      body: BlocListener<SchoolsBloc, SchoolsState>(
        listener: (context, state) {
          buildAlertDialog(context, title: '!!! Welcome to School Module !!!',
              content: 'Whole Module is developed with Flutter BLoc pattern and Integrated with Firebase realtime data base Rest apis');
          BlocProvider.of<SchoolsBloc>(context).updateWelcomeMessageStatus(
              true);
        },
        ...
        child: Column(
          ...
          children: [
            Wrap(
              ...
              children: [Text('Registered Schools:', style: Theme
                  .of(context)
                  .textTheme
                  .titleMedium),
    ...
    Widget _buildSyncButton() {
      return StreamBuilder<int>(
        ...
        builder: (context, snapshot) {
          ...
          return Badge(
              label: Text('$count'),
              child: ElevatedButton(onPressed: OfflineHandler().syncData,
      ...
      return StreamBuilder<OfflineDumpingStatus>(
        ...
        builder: (context, snapshot) {
          ...
          return ElevatedButton(
              onPressed: status == null ? onTapOfDumpStatus : () =>
                  onTapOfDumpStatus(true), child: child);
        },
      );
    }
    ...
    onTapOfEditSchool(SchoolViewModel school) {
      adaptiveDialog(context, CreateSchool(parentContext: context, school: school));
    }
