...
import 'package:sample_latest/core/local_server/model/callback_response_handler.dart';
  ...
  Future<Result<dynamic>> handleRequest(Request request) async {
    try {
      ...
      switch (request.url.path) {
        case 'motion':
          if (context.mounted) {
            final state = context.read<SmartControlDashboardCubit>().state;
            if (state is SCDashboardLoaded) {
              ...
              state.smCubits.entries.first.value
                  .updateDeviceStatus(result['motion'] ?? false);
            }
          }
